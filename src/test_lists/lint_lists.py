...
def lint_lists(lists_path, fix_duplicates=False, fix_slash=False):
    all_errors = []
    total_urls = 0
    total_countries = 0
    ...
    for csv_path in glob(os.path.join(lists_path, "*")):
        ...
        with open(csv_path, "r", encoding="utf-8") as in_file:
            ...
            if first_line != header:
                errors.append(InvalidHeader(csv_path, 0))
            urls_bag = set()
            errors = []
            rows = []
            duplicates = 0
            ...
            for idx, row in enumerate(reader):
                if len(row) != 6:
                    errors.append(InvalidColumnNumber(csv_path, idx + 2))
                    continue
                url, cat_code, cat_desc, date_added, source, notes = row
                err = check(url)
                if err:
                    errors.append(InvalidURL(url, csv_path, idx + 2, details=err))
                    if err == ERR_NOSLASH:
                        without_slash += 1
                        row[0] = row[0] + "/"
                ...
                try:
                    ...
                    errors.append(InvalidCategoryCode(cat_code, csv_path, idx + 2))
                if cat_description != cat_desc:
                    errors.append(InvalidCategoryDesc(cat_desc, csv_path, idx + 2))
                if url in urls_bag:
                    if not fix_duplicates:
                        errors.append(DuplicateURL(url, csv_path, idx + 2))
                    duplicates += 1
                    continue
                if not is_valid_date(date_added):
                    errors.append(InvalidDate(date_added, csv_path, idx + 2))
                if any([c in notes for c in BAD_CHARS]):
                    errors.append(InvalidNotes(notes, csv_path, idx + 2))
                if any([c in source for c in BAD_CHARS]):
                    errors.append(InvalidSource(source, csv_path, idx + 2))
                urls_bag.add(url)
                rows.append(row)
